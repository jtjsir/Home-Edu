...
<div style="position:relative;">
		...
		<div style="height: 250px;margin-top: 10px;">
			<div style="width:24%;float: left;text-align: center;">
				...
				<% 
					...
					String path = basePath + "/family/stu/photo?imgid=" +name ;
				%>
	...
	<div style="width:200px;position:absolute;right:0;">
		<div style="position:absolute;top:40px;left:-35px;">
			<p style="font-weight:600">其他相似学生</p>
			<div class="similarStus" style="margin-left:20px;"></div>
		</div>
		...
		success:function(data){
			...
			if(stuLen==0){
				...
				for(var j = 0; j <stuLen;j++){
					...
					var str = "<a href='<%=basePath%>/user/normal/stu/index?name="+ onestu.name + "'><p>" + "<lable>" + name + "</label></a>" + "<lable style='margin-left:10px;'>" + level + "</label>"+ "</p>" ;
					$('.similarStus').append(str) ;
...
<script type="text/javascript">
	$(function(){
		<%
			String stuname = ((User)request.getAttribute("normalUser")).getUsername() ;
			String teaname = ((User)request.getSession().getAttribute("user")).getUsername() ;
			int stuType = ((User)request.getAttribute("normalUser")).getType() ;
			int teaType = ((User)request.getSession().getAttribute("user")).getType() ;
		%>
		<%
			UserDetailStu detailUser = (UserDetailStu)request.getAttribute("detailUser") ;
			if(detailUser!=null){
